...
export const fetchExperiences = async () => {
  ...
  try {
    const supabase = createServerClient();
    const { data, error } = await supabase.from("experiences").select("*").eq("is_published", true);
    if (error) throw error;
    return { data, error: null };
  } catch (error) {
...
export const fetchEducation = async () => {
  ...
  try {
    const supabase = createServerClient();
    const { data, error } = await supabase.from("education").select("*").eq("is_published", true);
    if (error) throw error;
    return { data, error: null };
  } catch (error) {
...
export const fetchDashboards = async () => {
  // Try local data first
  const localData = await fetchLocalData('dashboards.json');
  if (localData) return { data: localData, error: null };
